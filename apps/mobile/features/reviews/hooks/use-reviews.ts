...
export const reviewsKeys = {
  ...
  mine: (mosqueId: string) => [...reviewsKeys.all, "mine", mosqueId] as const,
};
...
export function useMyReviewsForMosque(mosqueId: string | undefined) {
  ...
  return useQuery({
    ...
    queryFn: () => api.reviews.myForMosque({ mosqueId: mosqueId as string }),
...
export function useCreateReview() {
  ...
  return useMutation({
    mutationFn: (input: { mosqueId: string; rating: number; body?: string }) =>
